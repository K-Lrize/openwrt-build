...
# 4. 安装官方 Feed 软件包索引，确保 make defconfig 能解析官方包。
...
echo "::group::应用第三方覆盖"
{
    ...
    # @override 用于指定软件包来源 Feed。
    if [[ "$line" =~ ^#\ @override\ ([a-zA-Z0-9_-]+)\ ([a-zA-Z0-9_-]+) ]]; then
        feed="${BASH_REMATCH[1]}"
        pkg="${BASH_REMATCH[2]}"
        echo "  - 强制安装: $pkg [从 $feed]"
        ./scripts/feeds install -f -p "$feed" "$pkg" >/dev/null
    fi
done
echo "::endgroup::"

# 6. 准备最终 .config。必须放在 feeds 安装之后，避免被 OpenWrt 默认配置覆盖。
